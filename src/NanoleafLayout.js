...
export default class NanoleafLayout extends Component {
	...
	componentDidMount = () => {
		...
		this.props.data.positionData.map((value) => {
			this.drawTriangle(ctx, (value.x / PANEL_GAP_DIVISOR) + this.props.xOffset, (value.y / PANEL_GAP_DIVISOR) + this.props.yOffset, value.o, value.color);
		});
	};
	...
	render() {
		return (
			<div>
				<canvas id="canvas" width={this.props.canvasWidth} height={this.props.canvasHeight}> </canvas>
			</div>
		);
	}
}
